	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				//shadow = 1.0 - filmicGamma(1.0 - shadow);
				//shadow = GammaToLinearSpace(shadow);
		...
		Pass
		{
			...
			//RT0: diffuse color, occlusion(a)
			//RT1: spec color, roughness(a)
			void frag(v2f i, out float4 outDiffuse : SV_Target0, out float4 outSpecRoughness : SV_Target1)
			{
				...
				outDiffuse = float4(shadow, shadow, shadow, 1);
				outSpecRoughness = lerp(0.04, 1.0, shadow);

				//float3 col = 1. - (1. - shadow) / float3(0.1, 0.5, 0);
				//float3 col = 1. - (1. - shadow) * (1. - (float3(0.4, 0, 0) * shadow));
				//float3 col = shadow / (1. - float3(.05, 0., 0));
				//outDiffuse = float4(col, 1);
			}
